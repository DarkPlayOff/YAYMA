...
pub fn extract_native_flac(input_path: &str, output_path: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let extra_data = codec_params.extra_data.as_ref().ok_or("Missing FLAC STREAMINFO (extra_data)")?;
    ...
    header[0] = 0x80 | 0x00; // Флаг последнего блока | Тип 0 (StreamInfo)
