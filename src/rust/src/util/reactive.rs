...
fn register_effect(id: EffectId, entry: EffectEntry) {
    runtime().lock().effects.insert(id, entry);
}
...
impl<T: Send + Sync + 'static> Signal<T> {
    ...
    pub(crate) fn notify_subscribers(&self) {
        let subs = self.0.subscribers.lock().clone();
        for id in subs {
            schedule_effect(id);
        }
    }
...
impl<T: Clone + Send + Sync + 'static> Resource<T, String> {
        ...
        Fut: Future<Output = Result<T, String>> + Send + 'static,
    {
        ...
        create_effect(move |_| {
            ...
            *abort2.lock() = Some(handle.abort_handle());
        });
